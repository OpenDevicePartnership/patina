...
use core::{
    cell::UnsafeCell, cmp::Ordering, ptr
};
    ...
    D: SliceKey + 'a,
{
    ...
    pub const fn new() -> Self {
        Rbt { storage: Storage::new(), root: UnsafeCell::new(core::ptr::null_mut()) }
    }

    /// Creates a new binary tree with a given slice of memory.
    pub fn with_capacity(slice: &'a mut [u8]) -> Self {
        Rbt { storage: Storage::with_capacity(slice), root: UnsafeCell::new(core::ptr::null_mut()) }
    }
    ...
    fn root(&self) -> Option<&Node<D>> {
        let root_ptr = unsafe {self.root.get().read_volatile()};
        if root_ptr.is_null() {
            return None;
        }
    ...
    pub fn add(&mut self, data: D) -> Result<usize> {
        ...
        if unsafe {self.root.get().read_volatile().is_null() } {
            node.set_black();
            unsafe {self.root.get().write_volatile(node)};
            return Ok(idx);
        }

        let root = unsafe { &mut *self.root.get().read_volatile() };
    ...
    fn remove_node_from_tree<'b>(root: &'b UnsafeCell<*mut Node<D>>, to_delete: &'b Node<D>) {
        //} -> &'b Node<D> {
        ...
        if to_delete.left().is_none() && to_delete.right().is_none() {
            ...
            if to_delete.parent().is_none() {
                unsafe { root.get().write_volatile(ptr::null_mut()) };
            }
            return;
        }
        ...
        if to_delete.left().is_none() || to_delete.right().is_none() {
            ...
            if to_delete.parent().is_none() {
                unsafe { root.get().write_volatile(moved_up.as_mut_ptr()) };
        ...
        // if two children exist, find the successor and replace the value of the node, then removing the successor.
            ...
            if successor.parent().is_none() {
                unsafe { root.get().write_volatile(successor.as_mut_ptr()) };
    ...
    fn fixup_add(head: &UnsafeCell<*mut Node<D>>, node: &Node<D>) {
        ...
        // Parent is left child of grandparent
            ...
            if node.as_mut_ptr() == parent.right_ptr() {
                if let Some(root) = Self::rotate_left(parent)
                    && root.parent().is_none()
                {
                    unsafe { head.get().write_volatile(root.as_mut_ptr()) };
    ...
    fn fixup_delete(root: &UnsafeCell<*mut Node<D>>, node: Option<&Node<D>>) {
        // Case 1: The node is the root of the tree, no fixups needed.
        ...
        if sibling.is_red() {
            ...
            if node.parent().left_ptr() == node.as_mut_ptr() {
                ...
                {
                    unsafe { root.get().write_volatile(subtree_root.as_mut_ptr()) };
    ...
    D: SliceKey + Copy + 'a,
{
    ...
    pub fn resize(&mut self, slice: &'a mut [u8]) {
        let root = (!unsafe {self.root.get().read_volatile()}.is_null())
            .then(|| self.storage.idx(unsafe {self.root.get().read_volatile()}));

        self.storage.resize(slice);

        if let Some(idx) = root {
            unsafe { self.root.get().write_volatile(self.storage.get_mut(idx).expect("Pointer Exists.")) };
        }
    }
...
mod tests {
    ...
    fn test_delete_sibling_of_red() {
        ...
        */
        ...
        let root_ptr = UnsafeCell::new(root.as_mut_ptr());
        Rbt::<i32>::remove_node_from_tree(&root_ptr, left);

        let new_root = unsafe { &*(root_ptr.into_inner()) };
