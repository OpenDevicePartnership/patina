...
An example for the MADT follows:
...
impl TryFrom<AcpiTable> for AcpiMadt {
  fn try_from(table: &AcpiTable) {
      let local_apic_address = u32::from_le_bytes(table.data[0..4].try_into().unwrap());

      // ... additional parsing logic ...

      Ok(AcpiMadt { local_apic_address, flags, entries })
  }
}

fn try_into_example() {
  let table = AcpiTable::default();

  // .. parse common AcpiTable fields ...
  table.data = read_bytes(acpi_table_base_addr, acpi_table_length);

  let madt: AcpiMadt = table.try_into().expect("Invalid MADT");
}
```
...
# RANDOM STUFF DO NOT INCORPORATE YET

```rust
#[derive(IntoComponent)]
struct InstallMcfgComponent {
    private_config: McfgConfig,
}
...
impl InstallMcfgComponent {
    fn entry_point(self, config: ConfigMut<Config>, acpi_provider: Service<dyn AcpiProvider>) -> uefi_sdk::error::Result<()> {
      let segment_info = get_pci_segment_info(config.segment_count);
      let mcfg_table = AcpiTable::default();
      
      // fill in `mcfg_table` from `segment_info`
...
fn acpi_component() {
  ...
  let oem_revision = pcd_get_32(OEM_REVISION);
  let segment_count = pcd_get_32(SEGMENT_COUNT);

  let mcfg_component = InstallMcfgComponent::default()
                                            .with_config(McfgConfig { segment_count, oem_revision });
}
```
