...
```c
Status =  gBS->LocateProtocol(&gPatinaNonIdentityMappingProtocolGuid, NULL, (VOID**)&mPatinaNonIdentityMappingProtocol);
if (EFI_ERROR(Status)) {
  return Status;
}
...
mPatinaNonIdentityMappingProtocol->CreateAliasedMapping(
            mPatinaNonIdentityMappingProtocol,
            Va,
            Pa,
            Length,
            Attributes
...
```c
mPatinaNonIdentityMappingProtocol->UnmapAliasedMapping(
            mPatinaNonIdentityMappingProtocol,
            Va,
            Length
);
