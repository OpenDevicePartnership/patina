...
use crate::service::{
    SMBIOS_HANDLE_PI_RESERVED, SMBIOS_STRING_MAX_LENGTH, SmbiosHandle, SmbiosRecordsIter, SmbiosTableHeader, SmbiosType,
};
...
impl SmbiosManager {
    ...
    /// * `minor_version` - SMBIOS minor version (e.g., 9 for SMBIOS x.9)
    ///
    /// # Errors
    ///
    /// Returns `SmbiosError::UnsupportedVersion` if the major version is less than 3.
    /// Only SMBIOS 3.0 and above are supported.
    pub fn new(major_version: u8, minor_version: u8) -> Result<Self, SmbiosError> {
        if major_version < 3 {
            log::error!(
                "SMBIOS version {}.{} is not supported. Only SMBIOS 3.0 and above are supported.",
                major_version,
                minor_version
            );
    ...
    /// Parse strings from an SMBIOS string pool into a Vec<String>
    ///
    /// Extracts null-terminated strings from the string pool area and converts
    /// them to Rust String objects for manipulation.
    ///
    /// # Arguments
    ///
    ...
    /// # Returns
    ///
    /// Returns `Ok(Vec<String>)` with all strings from the pool, or an error if the pool is malformed.
    ///
    /// # Errors
    ///
    ...
    pub(super) fn parse_strings_from_pool(string_pool_area: &[u8]) -> Result<Vec<String>, SmbiosError> {
        ...
        // Split by null bytes and convert to Strings
        let strings: Result<Vec<String>, _> = data_without_terminator
            .split(|&b| b == 0)
            .map(|bytes| core::str::from_utf8(bytes).map(String::from).map_err(|_| SmbiosError::MalformedRecordHeader))
            .collect();
...
mod tests {
    ...
    use crate::error::SmbiosError;
    use crate::service::{SMBIOS_HANDLE_PI_RESERVED, SMBIOS_STRING_MAX_LENGTH, SmbiosHandle, SmbiosTableHeader};
    use r_efi::efi;
    ...
    fn test_version_custom_values() {
        let manager = SmbiosManager::new(4, 2).expect("failed to create manager");
        assert_eq!(manager.version(), (4, 2));
        let manager2 = SmbiosManager::new(255, 255).expect("failed to create manager");
