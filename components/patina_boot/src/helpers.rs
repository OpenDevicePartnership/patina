...
pub fn boot_from_device_path<B: BootServices>(boot_services: &B, device_path: &DevicePathBuf) -> Result<()> {
    // Get the DXE Core's image handle to use as the parent handle for LoadImage.
    // This is required per UEFI spec - the parent handle must be a valid image handle.
    let parent_handle = boot_services.dxe_core_image_handle().ok_or_else(|| {
        log::error!("DXE Core image handle not available");
        EfiError::NotReady
    })?;
...
mod tests {
    ...
    // Helper to create a mock with dxe_core_image_handle configured
    fn mock_with_dxe_core_handle() -> MockBootServices {
        let mut mock = MockBootServices::new();
        // Use a non-null dummy handle (1) to represent the DXE Core image handle
        mock.expect_dxe_core_image_handle().returning(|| Some(std::ptr::dangling_mut::<core::ffi::c_void>()));
        mock
    }
    ...
    fn test_boot_from_device_path_success() {
        ...
        let mut mock = mock_with_dxe_core_handle();
    ...
    #[test]
    fn test_boot_from_device_path_no_dxe_core_handle() {
        let device_path = create_test_device_path();
        let mut mock = MockBootServices::new();

        // Return None for dxe_core_image_handle - simulates not running under Patina DXE Core
        mock.expect_dxe_core_image_handle().returning(|| None);
