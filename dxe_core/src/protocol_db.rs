...
//!
//! protocol_db.install_protocol_interface(None, guid1, interface1).unwrap();
//!
//! assert_eq!(protocol_db.locate_protocol(guid1), Ok(interface1));
//! ```
//!
//! ## License
//!
...
const WELL_KNOWN_HANDLE_PROTOCOL_GUID: Uuid = Uuid::from_u128(0xfced7c96356e48cba9a9e089b2ddf49b);
...
impl ProtocolDb {
    ...
    pub fn init(&mut self) {
        let well_known_handle_guid: efi::Guid = unsafe {
            core::mem::transmute(*WELL_KNOWN_HANDLE_PROTOCOL_GUID.as_bytes())
        };
    ...
    /// # use std::str::FromStr;
    /// # use uuid::Uuid;
    /// use uefi_protocol_db::ProtocolDb;
    ///
    /// let mut protocol_db = ProtocolDb::new();
    /// let uuid1 = Uuid::from_str("0e896c7a-57dc-4987-bc22-abc3a8263210").unwrap();
    /// let guid1: efi::Guid = unsafe { core::mem::transmute(*uuid1.as_bytes()) };
    /// let interface1: *mut c_void = 0x1234 as *mut c_void;
    /// let (handle, notifies) = protocol_db.install_protocol_interface(None, guid1, interface1).unwrap();
    /// ```
    ...
    /// let (handle, notifies) = protocol_db.install_protocol_interface(None, guid1, interface1).unwrap();
    ///
    /// protocol_db.uninstall_protocol_interface(handle, guid1, interface1).unwrap();
    ///  ```
    ...
    /// let (handle, notifies) = protocol_db.install_protocol_interface(None, guid1, interface1).unwrap();
    /// let (handle2, notifies) = protocol_db.install_protocol_interface(None, guid1, interface2).unwrap();
    ///
    /// let handles = protocol_db.locate_handles(Some(guid1)).unwrap();
    /// assert!(handles.contains(&handle));
    /// assert!(handles.contains(&handle2));
    /// ```
    pub fn locate_handles(&mut self, protocol: Option<efi::Guid>) -> Result<Vec<efi::Handle>, efi::Status> {
    ...
    /// let (handle2, notifies) = protocol_db.install_protocol_interface(None, guid1, interface2).unwrap();
    ///
    /// let interface = protocol_db.locate_protocol(guid1).unwrap();
    ...
    /// let (handle2, notifies) = protocol_db.install_protocol_interface(None, guid1, interface2).unwrap();
    ///
    /// assert_eq!(interface1, protocol_db.get_interface_for_handle(handle, guid1).unwrap());
    /// assert_eq!(interface2, protocol_db.get_interface_for_handle(handle2, guid1).unwrap());
    /// ```
    ...
    /// let interface2: *mut c_void = 0x1234 as *mut c_void;
    /// let (handle, notifies) = protocol_db.install_protocol_interface(None, guid1, interface1).unwrap();
    /// let handle2 = (handle as usize + 1) as efi::Handle;
    ///
    /// assert_eq!(protocol_db.validate_handle(handle), Ok(()));
    /// assert_eq!(protocol_db.validate_handle(handle2), Err(efi::Status::INVALID_PARAMETER));
    /// ```
    ...
    /// let interface1: *mut c_void = 0x1234 as *mut c_void;
    /// let (handle, notifies) = protocol_db.install_protocol_interface(None, guid1, interface1).unwrap();
    /// let (agent_handle, notifies) = protocol_db.install_protocol_interface(None, guid1, interface1).unwrap();
    /// let (controller_handle, notifies) = protocol_db.install_protocol_interface(None, guid1, interface1).unwrap();
    ///
    /// protocol_db.add_protocol_usage(handle, guid1, Some(agent_handle), Some(controller_handle), efi::OPEN_PROTOCOL_BY_DRIVER).unwrap();
    /// ```
    ...
    /// protocol_db.add_protocol_usage(handle, guid1, Some(agent_handle), Some(controller_handle), efi::OPEN_PROTOCOL_BY_DRIVER).unwrap();
    ///
    /// protocol_db.remove_protocol_usage(handle, guid1, Some(agent_handle), Some(controller_handle)).unwrap();
    /// ```
    ...
    /// protocol_db.add_protocol_usage(handle, guid1, Some(agent_handle), Some(controller_handle), efi::OPEN_PROTOCOL_BY_DRIVER).unwrap();
    ///
    /// let open_protocol_info = protocol_db.get_open_protocol_information_by_protocol(handle, guid1).unwrap();
    ...
    /// protocol_db.add_protocol_usage(handle, guid1, Some(agent_handle), Some(controller_handle), efi::OPEN_PROTOCOL_BY_DRIVER).unwrap();
    ///
    /// let open_protocol_info = protocol_db.get_open_protocol_information(handle).unwrap();
    /// assert_eq!(open_protocol_info.len(), 1);
    ...
    /// assert_ne!(open_info[0].attributes & efi::OPEN_PROTOCOL_BY_DRIVER, 0);
    /// ```
    ...
    /// let (handle, notifies) = protocol_db.install_protocol_interface(None, guid1, interface1).unwrap();
    /// let (handle, notifies) = protocol_db.install_protocol_interface(Some(handle), guid2, interface2).unwrap();
    ///
    /// let protocols = protocol_db.get_protocols_on_handle(handle).unwrap();
    ///
    /// assert!(protocols.contains(&guid1));
    /// assert!(protocols.contains(&guid2));
    /// ```
    pub fn get_protocols_on_handle(&mut self, handle: efi::Handle) -> Result<Vec<efi::Guid>, efi::Status> {
        self.validate_handle(handle)?;
    ...
    /// let interface1: *mut c_void = 0x1234 as *mut c_void;
    ///
    /// let event: efi::Event = 0x4321 as efi::Event;
    ///
    /// let registration = protocol_db.register_protocol_notify(guid1, event);
    ///
    /// let (handle, notifies) = protocol_db.install_protocol_interface(None, guid1, interface1).unwrap();
    ///
    /// assert_eq!(notifies[0].event, event);
    /// ```
    pub fn register_protocol_notify(&mut self, protocol: efi::Guid, event: efi::Event) -> Result<*mut c_void, efi::Status> {
    ...
    /// assert_eq!(notifies.len(), 0);
    /// ```
    pub fn unregister_protocol_notify_events(&mut self, events: Vec<efi::Event>) {
        for event in events {
    ...
    /// let event: efi::Event = 0x4321 as efi::Event;
    ///
    /// let registration = protocol_db.register_protocol_notify(guid1, event).unwrap();
    ///
    /// let (handle, notifies) = protocol_db.install_protocol_interface(None, guid1, interface1).unwrap();
    /// let (handle2, notifies) = protocol_db.install_protocol_interface(None, guid1, interface1).unwrap();
    /// let (handle3, notifies) = protocol_db.install_protocol_interface(None, guid1, interface1).unwrap();
    ///
    /// assert_eq!(Some(handle), protocol_db.next_handle_for_registration(registration));
    /// assert_eq!(Some(handle2), protocol_db.next_handle_for_registration(registration));
    /// assert_eq!(Some(handle3), protocol_db.next_handle_for_registration(registration));
    /// assert_eq!(None, protocol_db.next_handle_for_registration(registration));
    /// ```
    pub fn next_handle_for_registration(&mut self, registration: *mut c_void) -> Option<efi::Handle> {
        for (_, v) in self.notifications.iter_mut() {
    ...
    /// let interface1: *mut c_void = 0x1234 as *mut c_void;
    ///
    /// let (controller, _) = protocol_db.install_protocol_interface(None, guid1, interface1).unwrap();
    /// let (driver, _) = protocol_db.install_protocol_interface(None, guid1, interface1).unwrap();
    /// let (child1, _) = protocol_db.install_protocol_interface(None, guid1, interface1).unwrap();
    /// let (child2, _) = protocol_db.install_protocol_interface(None, guid1, interface1).unwrap();
    /// let (child3, _) = protocol_db.install_protocol_interface(None, guid1, interface1).unwrap();
    /// let (_notchild1, _) = protocol_db.install_protocol_interface(None, guid1, interface1).unwrap();
    /// let (_notchild2, _) = protocol_db.install_protocol_interface(None, guid1, interface1).unwrap();
    ///
    /// for child in [child1, child2, child3] {
    ///   protocol_db
    ///     .add_protocol_usage(controller, guid1, Some(driver), Some(child), efi::OPEN_PROTOCOL_BY_CHILD_CONTROLLER)
    ///     .unwrap();
    /// }
    ///
    /// let child_list = protocol_db.get_child_handles(controller);
...
mod tests {
    ...
    fn new_should_create_protocol_db() {
        let protocol_db = ProtocolDb::new();
        assert_eq!(protocol_db.handles.len(), 0)
    }
    ...
    fn install_protocol_interface_should_install_protocol_interface() {
        ...
        let (handle, _) = protocol_db.install_protocol_interface(None, guid1, interface1).unwrap();
        ...
        let key = handle as usize;
        let protocol_instance = protocol_db.handles.get_mut(&key).unwrap();
    ...
    fn uninstall_protocol_interface_should_uninstall_protocol_interface() {
        ...
        assert!(protocol_db.handles.get_mut(&key).is_none());
    }
    ...
    fn uninstall_protocol_interface_should_give_access_denied_if_interface_in_use() {
        ...
        let mut instance =
        protocol_db.handles.get_mut(&key).unwrap().remove(&OrdGuid(guid1)).unwrap();
        ...
        protocol_db.handles.get_mut(&key).unwrap().insert(OrdGuid(guid1), instance);

        let err = protocol_db.uninstall_protocol_interface(handle, guid1, interface1);
    ...
    fn uninstall_protocol_interface_should_give_not_found_if_not_found() {
        ...
        let err = protocol_db.uninstall_protocol_interface(handle, guid2, interface1);
        assert_eq!(err, Err(efi::Status::NOT_FOUND));

        let err = protocol_db.uninstall_protocol_interface(handle, guid1, interface2);
        assert_eq!(err, Err(efi::Status::NOT_FOUND));
    }
    ...
    fn locate_handle_should_locate_handles() {
        ...
        assert_eq!(
            handle1,
            protocol_db.install_protocol_interface(Some(handle1), guid2, interface2).unwrap().0
        );
        let (handle2, _) = protocol_db.install_protocol_interface(None, guid1, interface1).unwrap();
        let (handle3, _) = protocol_db.install_protocol_interface(None, guid1, interface1).unwrap();
        let (handle4, _) = protocol_db.install_protocol_interface(None, guid1, interface1).unwrap();
        assert_eq!(
            handle4,
            protocol_db.install_protocol_interface(Some(handle4), guid2, interface2).unwrap().0
        );

        let handles = protocol_db.locate_handles(None).unwrap();
        for handle in [handle1, handle2, handle3, handle4] {
            assert!(handles.contains(&handle));
        }

        let handles = protocol_db.locate_handles(Some(guid2)).unwrap();
        ...
        assert_eq!(protocol_db.locate_handles(Some(guid3)), Err(efi::Status::NOT_FOUND));
    }
    ...
    fn validate_handle_should_validate_good_handles_and_reject_bad_ones() {
        ...
        assert_eq!(protocol_db.validate_handle(handle1), Ok(()));
    ...
    fn validate_handle_empty_handles_are_invalid() {
        ...
        let (handle1, _) = protocol_db.install_protocol_interface(None, guid1, interface1).unwrap();
        protocol_db.uninstall_protocol_interface(handle1, guid1, interface1).unwrap();
        assert_eq!(protocol_db.validate_handle(handle1), Err(efi::Status::INVALID_PARAMETER));
    }
    ...
    fn add_protocol_usage_by_child_controller() {
        ...
        let result = protocol_db.add_protocol_usage(
            handle1,
            guid1,
            None,
            None,
    ...
    fn test_handle_get_or_test_protocol_usage(test_attributes: u32) {
        ...
        protocol_db.add_protocol_usage(handle1, guid1, None, Some(handle3), test_attributes).unwrap();
        let protocol_user_list =
            &protocol_db.handles.get(&(handle1 as usize)).unwrap().get(&OrdGuid(guid1)).unwrap().usage;
        assert_eq!(1, protocol_user_list.len());
        assert_eq!(1, protocol_user_list[0].open_count);
        ...
        protocol_db.add_protocol_usage(handle1, guid1, None, None, test_attributes).unwrap();
        let protocol_user_list =
            &protocol_db.handles.get(&(handle1 as usize)).unwrap().get(&OrdGuid(guid1)).unwrap().usage;
        assert_eq!(1, protocol_user_list.len());
        assert_eq!(1, protocol_user_list[0].open_count);
        ...
        protocol_db.add_protocol_usage(handle1, guid1, Some(handle2), None, test_attributes).unwrap();
        let protocol_user_list =
            &protocol_db.handles.get(&(handle1 as usize)).unwrap().get(&OrdGuid(guid1)).unwrap().usage;
        assert_eq!(2, protocol_user_list.len());
        assert_eq!(1, protocol_user_list[0].open_count);
        ...
        protocol_db.add_protocol_usage(handle4, guid1, Some(handle2), None, test_attributes).unwrap();
        let protocol_user_list =
            &protocol_db.handles.get(&(handle1 as usize)).unwrap().get(&OrdGuid(guid1)).unwrap().usage;
        assert_eq!(2, protocol_user_list.len());
        assert_eq!(1, protocol_user_list[1].open_count);
    ...
    fn remove_protocol_usage_should_succeed_regardless_of_attributes() {
        ...
        let (agent, _) = protocol_db.install_protocol_interface(None, guid1, interface1).unwrap();
        let (controller, _) = protocol_db.install_protocol_interface(None, guid1, interface1).unwrap();
        ...
        for attributes in [
            ...
            protocol_db
                ...
                .unwrap();
            protocol_db.remove_protocol_usage(handle, guid1, Some(agent), Some(controller)).unwrap();
            let protocol_user_list =
    ...
    fn remove_protocol_usage_should_return_not_found_if_usage_not_found() {
        ...
        let result = protocol_db.remove_protocol_usage(handle1, guid1, Some(handle3), Some(handle2));
        assert_eq!(result, Err(efi::Status::NOT_FOUND));
        let protocol_user_list =
            &protocol_db.handles.get(&(handle1 as usize)).unwrap().get(&OrdGuid(guid1)).unwrap().usage;
        assert_eq!(1, protocol_user_list.len());

        let result = protocol_db.remove_protocol_usage(handle1, guid1, None, Some(handle3));
        assert_eq!(result, Err(efi::Status::NOT_FOUND));
        let protocol_user_list =
            &protocol_db.handles.get(&(handle1 as usize)).unwrap().get(&OrdGuid(guid1)).unwrap().usage;
        assert_eq!(1, protocol_user_list.len());

        let result = protocol_db.remove_protocol_usage(handle1, guid1, Some(handle2), None);
        assert_eq!(result, Err(efi::Status::NOT_FOUND));
        let protocol_user_list =
            &protocol_db.handles.get(&(handle1 as usize)).unwrap().get(&OrdGuid(guid1)).unwrap().usage;
        assert_eq!(1, protocol_user_list.len());

        let result = protocol_db.remove_protocol_usage(handle1, guid1, None, None);
        assert_eq!(result, Err(efi::Status::NOT_FOUND));
        let protocol_user_list =
            &protocol_db.handles.get(&(handle1 as usize)).unwrap().get(&OrdGuid(guid1)).unwrap().usage;
        assert_eq!(1, protocol_user_list.len());
    ...
    fn add_protocol_usage_should_succeed_after_remove_protocol_usage() {
        ...
        assert_eq!(
            protocol_db.add_protocol_usage(
                handle1,
                guid1,
                Some(handle4),
                Some(handle3),
        ...
        protocol_db.remove_protocol_usage(handle1, guid1, Some(handle2), Some(handle3)).unwrap();
    ...
    fn get_open_protocol_information_by_protocol_returns_information() {
        ...
        let open_protocol_info_list =
            protocol_db.get_open_protocol_information_by_protocol(handle, guid1).unwrap();
    ...
    fn get_open_protocol_information_by_protocol_should_return_not_found_if_handle_or_protocol_not_present() {
        ...
        let (handle, _) = protocol_db.install_protocol_interface(None, guid1, interface1).unwrap();
        let (handle2, _) = protocol_db.install_protocol_interface(None, guid2, interface2).unwrap();
        let (agent, _) = protocol_db.install_protocol_interface(None, guid1, interface1).unwrap();
        let (controller, _) = protocol_db.install_protocol_interface(None, guid1, interface1).unwrap();
        ...
        let result = protocol_db.get_open_protocol_information_by_protocol(handle, guid2);
        assert_eq!(result, Err(efi::Status::NOT_FOUND));

        let result = protocol_db.get_open_protocol_information_by_protocol(handle2, guid1);
        assert_eq!(result, Err(efi::Status::NOT_FOUND));
    }
    ...
    fn to_efi_open_protocol_should_match_source_open_protocol_information_entry() {
        ...
        for info in protocol_db.get_open_protocol_information_by_protocol(handle, guid1).unwrap() {
    ...
    fn get_open_protocol_information_should_return_all_open_protocol_info() {
        ...
        let open_protocol_info_list = protocol_db.get_open_protocol_information(handle).unwrap();
        assert_eq!(attributes_list.len(), test_info.len());
    ...
    fn get_interface_for_handle_should_return_the_interface() {
        ...
        let returned_interface = protocol_db.get_interface_for_handle(handle, guid1).unwrap();
    ...
    fn get_protocols_on_handle_should_return_protocols_on_handle() {
        ...
        let (handle, _) = protocol_db.install_protocol_interface(None, guid1, interface1).unwrap();
        protocol_db.install_protocol_interface(Some(handle), guid2, interface2).unwrap();

        let protocol_list = protocol_db.get_protocols_on_handle(handle).unwrap();
    ...
    fn locate_protocol_should_return_protocol() {
        ...
        protocol_db.install_protocol_interface(None, guid1, interface1).unwrap();
        protocol_db.install_protocol_interface(None, guid2, interface2).unwrap();

        assert_eq!(protocol_db.locate_protocol(guid1), Ok(interface1));
        assert_eq!(protocol_db.locate_protocol(guid2), Ok(interface2));
    }
    ...
    fn register_protocol_notify_should_register_protocol_notify() {
        ...
        let result = protocol_db.register_protocol_notify(guid1, event);
        assert!(result.is_ok());
        assert!(!result.unwrap().is_null());

        {
            let notifications = &protocol_db.notifications;
        ...
        let result = protocol_db.register_protocol_notify(guid1, event2);
        assert!(result.is_ok());
        assert!(!result.unwrap().is_null());
    ...
    fn install_protocol_interface_should_return_registered_notifies() {
        ...
        let reg1 = protocol_db.register_protocol_notify(guid1, event).unwrap();
        let event2 = 0x4321 as *mut c_void;
        let reg2 = protocol_db.register_protocol_notify(guid1, event2).unwrap();

        let result = protocol_db.install_protocol_interface(None, guid1, interface1);
        assert!(result.is_ok());
    ...
    fn unregister_protocol_notifies_should_unregister_protocol_notifies() {
        ...
        let event = 0x8765 as *mut c_void;
        protocol_db.register_protocol_notify(guid1, event).unwrap();
        let event2 = 0x4321 as *mut c_void;
        protocol_db.register_protocol_notify(guid1, event2).unwrap();

        let (_, notifies) = protocol_db.install_protocol_interface(None, guid1, interface1).unwrap();

        let events = notifies.iter().map(|x| x.event).collect();

        protocol_db.unregister_protocol_notify_events(events);
    ...
    fn next_handle_for_registration_should_return_next_handle_for_registration() {
        ...
        let hnd1 = protocol_db.install_protocol_interface(None, guid1, interface1).unwrap().0;
        let hnd2 = protocol_db.install_protocol_interface(None, guid1, interface1).unwrap().0;
        let hnd3 = protocol_db.install_protocol_interface(None, guid1, interface1).unwrap().0;

        let result = protocol_db.next_handle_for_registration(reg1);
        ...
        let result = protocol_db.next_handle_for_registration(reg2);
        assert!(result.is_some());
        assert_eq!(result.unwrap(), hnd1);
