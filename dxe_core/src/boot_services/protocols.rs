...
use crate::protocol_db::{DXE_CORE_HANDLE, ProtocolDb};
use core::{slice, mem::size_of, ffi::c_void};
use crate::{event_db::EventDb, allocator::core_allocate_pool};

use super::{driver, SystemState};

pub fn core_install_protocol_interface(
    protocol_db: &mut ProtocolDb,
    event_db: &mut EventDb,
    st: &super::SystemState,
    ...
    for notify in notifies {
        if super::events::signal_event(event_db, st, notify.event) == efi::Status::INVALID_PARAMETER {
...
pub fn install_protocol_interface(
    ...
    let installed_handle = match core_install_protocol_interface(protocol_db, event_db, st, caller_handle, caller_protocol, interface) {
        Err(err) => return err,
        Ok(handle) => handle,
    };
...
pub fn uninstall_protocol_interface(
    ...
    loop {
        ...
        for usage in usages {
            if (usage.attributes & efi::OPEN_PROTOCOL_BY_DRIVER) != 0 {
                ...
                unsafe {
                    usage_close_status = driver::core_disconnect_controller(protocol_db, handle, usage.agent_handle, None);
    ...
    if usage_close_status.is_ok() {
        ...
        for usage in usages {
            if usage.attributes & (efi::OPEN_PROTOCOL_BY_HANDLE_PROTOCOL | efi::OPEN_PROTOCOL_GET_PROTOCOL | efi::OPEN_PROTOCOL_TEST_PROTOCOL) != 0 {
               let result = protocol_db.remove_protocol_usage(handle, caller_protocol, usage.agent_handle, usage.controller_handle);
               if result.is_err() {
    ...
    if usage_close_status.is_err() || unclosed_usages {
        unsafe {
            let _result = driver::core_connect_controller(protocol_db, handle, Vec::new(), None, true);
        }
    }
...
const PRIVATE_DUMMY_INTERFACE_GUID: efi::Guid =
    ...
    #[inline(always)]
fn install_dummy_interface(protocol_db: &mut ProtocolDb, handle: efi::Handle) -> Result<(), efi::Status> {
    protocol_db
...
#[inline(always)]
fn uninstall_dummy_interface(protocol_db: &mut ProtocolDb, handle: efi::Handle) -> Result<(), efi::Status> {
...
pub fn reinstall_protocol_interface(
    ...
    // invalid handle).
    if let Err(err) = install_dummy_interface(protocol_db, handle) {
        return err;
    }

    match uninstall_protocol_interface(protocol_db, handle, protocol, old_interface) {
        efi::Status::SUCCESS => (),
        err => {
    ...
    if let Err(err) = core_install_protocol_interface(protocol_db, event_db, st, Some(handle), protocol, new_interface) {
        let result = uninstall_dummy_interface(protocol_db, handle);
        debug_assert!(result.is_ok());
        return err;
    }
    ...
    // status is ignored.
    unsafe {
        let _ = driver::core_connect_controller(protocol_db, handle, Vec::new(), None, true);
    }
...
pub fn open_protocol(
    ...
    if (attributes & efi::OPEN_PROTOCOL_EXCLUSIVE) != 0 {
        ...
        if let Some(usage) = usages.iter().find(|x| {
            ...
            unsafe {
                if driver::core_disconnect_controller(protocol_db, handle, usage.agent_handle, None).is_err() {
...
pub unsafe extern "C" fn install_multiple_protocol_interfaces(
    ...
    let mut interfaces_to_install = alloc::vec::Vec::new();
    loop {
    ...
    install_multiple_protocol_interfaces_core(&mut super::BOOT_SERVICES.protocol_db.lock(), &mut super::BOOT_SERVICES.event_db.lock(), &super::BOOT_SERVICES.system_state, handle, interfaces_to_install.as_slice())
}

#[inline(always)]
pub fn install_multiple_protocol_interfaces_core(
    protocol_db: &mut ProtocolDb,
    event_db: &mut EventDb,
    st: &super::SystemState,
    handle: *mut efi::Handle,
    interfaces: &[(*mut efi::Guid, *mut c_void)],
) -> efi::Status {
    let mut interfaces_to_uninstall_on_error = Vec::new();
    for (protocol, interface) in interfaces {
        match install_protocol_interface(protocol_db, event_db, st, handle, *protocol, efi::NATIVE_INTERFACE, *interface) {
            ...
            err => {
                for (protocol, interface) in interfaces_to_uninstall_on_error {
                    let _ = uninstall_protocol_interface(protocol_db, unsafe { *handle }, *protocol, *interface);
                }
                return err;
            }
        }
...
pub unsafe extern "C" fn uninstall_multiple_protocol_interfaces(
    ...
    uninstall_multiple_protocol_interfaces_core(&mut super::BOOT_SERVICES.protocol_db.lock(), &mut super::BOOT_SERVICES.event_db.lock(), &super::BOOT_SERVICES.system_state, handle, interfaces_to_uninstall.as_slice())
}

#[inline(always)]
fn uninstall_multiple_protocol_interfaces_core(
    protocol_db: &mut ProtocolDb,
    event_db: &mut EventDb,
    st: &SystemState,
    handle: efi::Handle,
    ...
    for (protocol, interface) in interfaces {
        match uninstall_protocol_interface(protocol_db, handle, *protocol, *interface) {
            ...
            _ => {
                for (protocol, interface) in interfaces_to_reinstall_on_error {
                    let protocol = *(unsafe { protocol.as_mut().expect("previously null-checked pointer is null.") });
                    let _ = core_install_protocol_interface(protocol_db, event_db, st, Some(handle), protocol, *interface);
                }
                return efi::Status::INVALID_PARAMETER;
            }
        }
...
pub fn core_locate_device_path(
    ...
    for handle in handles {
        ...
        let status = handle_protocol(protocol_db, handle, device_path_protocol_guid, temp_device_path_ptr);
...
pub fn locate_device_path(
    ...
    let (best_remaining_path, best_device) =
        match core_locate_device_path(protocol_db, unsafe { *protocol }, unsafe { *device_path }) {
            Err(err) => return err,
