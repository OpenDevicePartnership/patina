...
impl PrivateImageData {
    fn new(image_info: efi::protocols::loaded_image::Protocol, pe_info: &UefiPeInfo) -> Self {
        PrivateImageData {
...
fn apply_image_memory_protections(pe_info: &UefiPeInfo, private_info: &PrivateImageData) {
    for section in &pe_info.sections {
        ...
        // attributes. We also need to ensure the capabilities are set. We set the capabilites as the old capabilites
...
fn install_dxe_core_image(hob_list: &HobList) {
    ...
    let mut private_image_data = PrivateImageData::new(image_info, &Default::default());
