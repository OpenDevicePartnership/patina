...
/// One entry per install-notify dispatch currently executing, outermost first.
struct ActiveInstallNotify {
    /// The `NotifyHolder` context of the dispatch running at this stack depth.
    context: *mut c_void,
    /// Set when this dispatch cancels its own registration, so the notify install trampoline
    /// frees the holder once its dispatch loop returns instead of while it is still executing.
    close_requested: bool,
}

// SAFETY: `ActiveInstallNotify` values are only ever pushed, inspected, and popped while holding
// `ACTIVE_INSTALL_NOTIFIES`'s lock.
unsafe impl Send for ActiveInstallNotify {}

/// Tracks install-notify contexts currently executing. Used to detect a dispatch cancelling its own
/// registration (or an ancestor's, if cancelled from a nested notification) so the holder is not freed
/// while it is still executing.
static ACTIVE_INSTALL_NOTIFIES: TplMutex<Vec<ActiveInstallNotify>> =
    TplMutex::new(efi::TPL_HIGH_LEVEL, Vec::new(), "ActiveInstallNotifies");
...
extern "efiapi" fn notify_install_trampoline(_event: efi::Event, context: *mut c_void) {
    ...
    ACTIVE_INSTALL_NOTIFIES.lock().push(ActiveInstallNotify { context, close_requested: false });
    ...
    // This is guaranteed to be the entry pushed above. Entries are pushed and popped in call order,
    // so nothing pushed after it would still be on the stack at this point.
    let popped = ACTIVE_INSTALL_NOTIFIES.lock().pop();
    let close_requested = if let Some(active) = popped {
        debug_assert_eq!(active.context, context, "active install-notify stack popped out of order");
...
impl ProtocolServices for CoreProtocolServices {
    ...
    fn cancel_install_notify(&self, registration: NotifyRegistration) -> Result<(), ProtocolError> {
        ...
        if !context.is_null() {
            let mut active_notifies = ACTIVE_INSTALL_NOTIFIES.lock();
            let deferred = if let Some(active) = active_notifies.iter_mut().rev().find(|entry| entry.context == context)
            {
                // This registration is cancelling itself, or an ancestor registration if cancelled
                // from a nested notification. The notify install trampoline frees the holder when the
