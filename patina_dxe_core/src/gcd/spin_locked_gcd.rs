        ...
        if let Some(page_table) = &mut *self.page_table.lock() {
            ...
            if paging_attrs & MemoryAttributes::ReadProtect == MemoryAttributes::ReadProtect {
                if unmapped {
                    ...
                    log::trace!(
                        target: "paging",
                        "Memory region {base_address:#x?} of length {len:#x?} with attributes {attributes:#x?}. No paging action taken: Region already unmapped",
                    );
                    return Ok(());
                }
                ...
                match self.set_paging_attributes(current_base as usize, current_len as usize, attributes) {
                    ...
                    Err(e) => {
                            ...
                            .is_err()
                        {
                            ...
                            log::error!(
                                "Failed to roll back GCD attributes after page table attribute set failure. This is a critical error. GCD and page table are now out of sync."
                            );
                        }
